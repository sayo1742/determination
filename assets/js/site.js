(function(){
  ...
  if(sidebarCounter){
    ...
    sidebarCounter.textContent = String(days).padStart(4,'0');
  }
  ...
  function post(id){
    ...
    currentPostId = p.id;
    const body = p.password ? `<div class="password-box"><strong>🔒 PASSWORD CONTENT</strong><div class="locked" id="lockedPreview"><p>${esc(p.summary)}</p><p>████████████████</p></div><div class="password-row"><input id="passwordInput" type="password" placeholder="輸入密碼"><button id="unlockBtn">UNLOCK</button></div></div>` : `<div class="single-post-body">${p.content}</div>`;
  ...
  function gallery(){
    ...
    transition(`<div class="page-wrap"><div class="page-title">GALLERY</div><section class="content-panel"><div class="gallery-grid">${items.map(item=>item.password
      ? `<button class="gallery-item gallery-secret" data-gallery-src="${esc(item.src)}" data-gallery-credit="${esc(item.credit||'')}" data-gallery-password="${esc(item.password)}"><span>secret</span></button>`
      : `<button class="gallery-item gallery-photo" data-gallery-src="${esc(item.src)}" data-gallery-credit="${esc(item.credit||'')}"><img src="${esc(item.thumb||item.src)}" alt="Gallery image"></button>`).join('')}</div></section></div>`,'gallery');
    document.querySelectorAll('[data-gallery-src]').forEach(el=>el.onclick=()=>{
      const pass=el.dataset.galleryPassword||'';
      if(pass) openLockedLightbox(el.dataset.gallerySrc, el.dataset.galleryCredit, pass);
      else openLightbox(el.dataset.gallerySrc, el.dataset.galleryCredit);
    });
  }
  const characterProfiles = {
    A: {
      family:'MORIHARA', given:'AOI', jp:'森原 あおい', sub:'-', images:['assets/images/character-a-standee.png','assets/images/character-a-standee-b.png'], accent:'#6b897e',
      seasons: {
        '1': {
    ...
    B: {
      family:'HIKAWA', given:'SAYO', jp:'氷川 紗夜', sub:'Roselia / Gt.', images:['assets/images/character-b-standee.png','assets/images/character-b-standee-b.png'], accent:'#93b2c6',
      seasons: {
        '1': {
  ...
  function characterPage(which){
    ...
    transition(`<div class="page-wrap">
      <section class="character-profile-v16 character-profile-v17" style="--character-accent:${profile.accent}">
        ...
        <div class="character-profile-visual character-profile-visual-v17">
          <img id="characterStandee" src="${profile.images[0]}" alt="${profile.jp}">
          <div class="character-visual-switch" aria-label="立繪切換">
            <button class="active" data-visual-index="0" type="button">01</button>
            <button data-visual-index="1" type="button">02</button>
          </div>
        </div>
      </section></div>`,'character');
    ...
    document.querySelectorAll('[data-visual-index]').forEach(btn=>btn.onclick=()=>{
      const idx=Number(btn.dataset.visualIndex);
      const img=document.getElementById('characterStandee');
      if(img && profile.images[idx]) img.src=profile.images[idx];
      document.querySelectorAll('[data-visual-index]').forEach(b=>b.classList.toggle('active',b===btn));
    });
  ...
  function characterStory(){
    transition(`<div class="page-wrap">
      <section class="story-lock-screen">
        <button class="back-link story-back" id="backCharacter">← CHARACTER</button>
        <form class="story-lock-form" id="storyLockForm" autocomplete="off">
          <input id="storyPassword" type="password" inputmode="numeric" maxlength="4" pattern="[0-9]{4}" aria-label="4-digit password" placeholder="••••">
          <button type="submit" aria-label="送出密碼">→</button>
        </form>
      </section></div>`,'character');
    document.getElementById('backCharacter').onclick=character;
    const form=document.getElementById('storyLockForm');
    form.onsubmit=(e)=>{
      e.preventDefault();
      if(document.getElementById('storyPassword').value==='0615') renderCharacterStory();
      else {
  ...
  function renderCharacterStory(){
    transition(`<div class="page-wrap">
      <section class="character-story-v22">
        <header class="character-story-head-v22">
          <h1>ABOUT THEM</h1>
          <button class="back-link" id="backCharacter">← CHARACTER</button>
        </header>
        <div class="story-season-timeline">
          <section class="story-season-card left"><span class="story-dot" aria-hidden="true"></span><h2>シーズン1</h2><div class="story-season-empty"></div></section>
          <section class="story-season-card right"><span class="story-dot" aria-hidden="true"></span><h2>シーズン2</h2><div class="story-season-empty"></div></section>
          <section class="story-season-card left"><span class="story-dot" aria-hidden="true"></span><h2>シーズン3</h2><div class="story-season-empty"></div></section>
        </div>
      </section></div>`,'character');
    document.getElementById('backCharacter').onclick=character;
  }
  ...
  function garupa(){
    ...
    transition(`<div class="page-wrap garupa-page">
      ...
      <section class="garupa-stats" aria-label="遊戲資料">
        ...
        <div><span>ALBUM</span><strong>${esc(g.sayoCollection||'—')}</strong></div>
  ...
  function garupaEvent(id){
    ...
    transition(`<div class="page-wrap garupa-page"><div class="page-title">GARUPA ARCHIVE</div>
      <section class="garupa-detail">
        ...
        ${(e.screenshots||[]).length?`<div class="garupa-screenshot-grid">${e.screenshots.map((src,i)=>`<button class="garupa-shot" data-shot="${esc(src)}"><img src="${esc(src)}" alt="活動截圖 ${i+1}"></button>`).join('')}</div>`:''}
      </section></div>`,'garupa');
  ...
  function garupaCard(id){
    ...
    transition(`<div class="page-wrap garupa-page"><div class="page-title">GARUPA ARCHIVE</div>
      <section class="garupa-detail">
        ...
        <div class="sayo-card-detail-empty"></div>
      </section></div>`,'garupa');
  ...
  function openLightbox(src, credit=''){
    ...
    const lock=document.getElementById('galleryLock');
  ...
  function openLockedLightbox(src, credit='', password='0615'){
    if(!lightbox || !src) return;
    const img=lightbox.querySelector('img');
    const lock=document.getElementById('galleryLock');
    const input=document.getElementById('galleryPassword');
    const unlock=document.getElementById('galleryUnlock');
    const creditEl=document.getElementById('lightboxCredit');
    img.hidden=true;
    img.src='';
    ...
    const submit=()=>{
      if(input && input.value===password){
        ...
        if(creditEl){ creditEl.textContent=credit||''; creditEl.hidden=!credit; }
      } else if(lock){ lock.classList.remove('shake'); void lock.offsetWidth; lock.classList.add('shake'); }
    };
    if(unlock) unlock.onclick=submit;
  ...
  function route(name){({home,posting,post:()=>post(currentPostId),gallery,character,garupa}[name]||home)();}
